// "7 cities everyone warns you about" carousel - myth vs verified reality,
// same DNA as build-myths.mjs but per-city. Facts verified 20.07. via
// real solo-female-travel reports (Nomadic Matt, Travel Ladies, Adventurous
// Kate, Where Goes Rose, The Lady Who Travels, and others) - not fabricated.
// Johannesburg is deliberately NOT myth-busted into "it's fine actually" -
// the caution there is real; brand honesty over hype.
// 9 slides: cover + 7 cities + endcard, 1080x1920 (9:16).
// Usage: node build-cities-warning.mjs
...
const OUT = join(__dirname, 'out', 'cities-warning');
...
.scrim{position:absolute;inset:0;background:linear-gradient(180deg, rgba(6,29,21,.55) 0%, rgba(6,29,21,.3) 38%, rgba(6,29,21,.14) 68%, rgba(6,29,21,.4) 100%)}
...
CITIES.forEach((c, i) => {
  ...
  <div class="pad">
    ...
    <div class="bottom"><span></span>${i < CITIES.length - 1 ? SWIPE : ''}</div>
  </div></div>`);
});
// 9 endcard
