# IF IT HAPPENS 02 - "Your hotel cancels without warning"

**Anlass:** Ein Reel eines Mädchens erzählt die Situation (Storno ohne Vorwarnung),
aber ohne Lösung. Wir liefern die Lösung - 5 echte Schritte.
**Format:** TikTok-Carousel (7 Slides, 1080x1920) + IG-Reel (18,8 s, gleiche
7 Fotos mit Ken-Burns-Zoom, ohne Ton).
**Fakten verifiziert 16.07.** (One Mile at a Time, Perk, Daily Passport):
Ein Hotel darf stornieren, aber "non-refundable" bindet nur den Gast - storniert
das Hotel, gibt es immer eine volle Rückerstattung. Buchungsplattformen schulden
eine vergleichbare Alternative, oft inkl. erster Nacht.

## Die 5 Schritte

1. **Screenshot everything.** Buchungsbestätigung, Storno-Nachricht, Chat - das
   ist der Beweis, sofort sichern.
2. **"Non-refundable" was never your rule.** Das Label bindet nur dich als
   Stornierende - storniert das Hotel, gibt's immer volle Rückerstattung.
3. **Call the platform, not just the hotel.** Booking.com/Airbnb & Co. schulden
   eine vergleichbare Alternative, oft inkl. erster Nacht in der neuen Unterkunft.
4. **Save 3 backups before you fly.** 2-3 Alternativ-Hotels vor der Reise
   screenshotten - "kein Zimmer" wird zur 90-Sekunden-Lösung statt Panik.
5. **Walk into a different lobby.** Persönlich vor Ort schneller als Hotline
   um Mitternacht.

## Caption (IG + TikTok)

> Your hotel just canceled. Tonight. Here's what actually works - not just
> what happened to someone else.
>
> 1. Screenshot everything - confirmation, cancellation message, chat
> 2. You still get your money back - "non-refundable" only stops YOU from canceling
> 3. Call the platform, not just the hotel - they owe you a comparable room
> 4. Save 2-3 backup hotels before you even fly
> 5. If the phone gets you nowhere, just walk into another hotel and ask - a person at a desk sorts it out faster
>
> A canceled room ends a booking, not a trip. Save this before you need it. 💚
>
> #solofemaletravel #solotravel #traveltips #hoteltips #fyp

## Produktion

- **Carousel:** `daya/studio/build-hotel-canceled.mjs`, 7 Fotos in
  `photos/hotel-canceled/` (nano-banana-pro, 14 Credits, faceless/von hinten/
  Hände).
- **Reel:** `daya/studio/build-reel-hotel-canceled.mjs` - identische 7 Fotos,
  Ken-Burns-Zoom + Overlays, 0 zusätzliche Credits.
- Ton: Trending Sound in der App, oder ElevenLabs-VO (Text unten) +
  Caption AI, analog Reel 03.

## ElevenLabs-Voiceover (Option)

> Your hotel just canceled. Tonight. Here's what actually works.
> Step one: screenshot everything. The confirmation, the cancellation message, the chat.
> Step two: you still get your money back. "Non-refundable" only stops you from canceling - if the hotel cancels, they owe you a full refund.
> Step three: call the platform, not just the hotel. They owe you a comparable room.
> Step four: save two or three backup hotels - before you even fly.
> Step five: if the phone gets you nowhere, just walk into another hotel. A person at a desk helps faster than any helpline.
> A canceled room ends a booking. Not a trip.

## Brand-Guardian-Check

Fakten belegt (Rückerstattungsrecht, Plattform-Pflichten) ✅ · faceless ✅ ·
kein Angstmarketing (Ton: Lösung, nicht Drama) ✅ · ≤5 Hashtags ✅ · 💚 ✅ ·
kein Playbook-CTA ✅ · kein Logo, nur Pfeil-Stil ✅.
